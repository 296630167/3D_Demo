...
public abstract class 基: MonoBehaviour
{
    ...
    // 二次封装下 启动 和 关闭携程的方法 
    public virtual Coroutine 启动携程(IEnumerator 携程) => Mono方法管理器.开启协程(携程);
    public virtual void 关闭携程(Coroutine 携程) => Mono方法管理器.结束协程(携程);
    public virtual Coroutine 启动携程(string 携程名称) => Mono方法管理器.开启协程(携程名称);
    public virtual void 关闭携程(string 携程名称) => Mono方法管理器.结束协程(携程名称);
}