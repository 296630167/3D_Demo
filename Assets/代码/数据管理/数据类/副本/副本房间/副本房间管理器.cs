...
public class 副本房间管理器
{
    ...
    public List<副本房间_基类> 可分配房间列表;

    public 副本房间管理器(int 副本大小分行 = 5, int 副本大小分列 = 5)
    {
        副本房间行数 = 副本大小分行;
        副本房间列数 = 副本大小分列;
        副本房间数组 = new 副本房间_基类[副本大小分行, 副本大小分列];
        初始化副本房间数据();
    }
    ...
    private List<副本房间_基类> 获取周边房间(副本房间_基类 房间) => new List<副本房间_基类> { 周边房间_上(房间), 周边房间_下(房间), 周边房间_左(房间), 周边房间_右(房间) };

    private void 初始化副本房间数据()
    {
        可分配房间列表 = new List<副本房间_基类>();
        初始化每个房间();
        初始化入口和出口();
        初始化副本主路径();
    }
    ...
    private struct 可扩展格子
    {
        ...
        public int 插入索引;
        public 可扩展格子(List<副本房间_基类> 主路径格子, List<副本房间_基类> 扩展格子, int 插入索引)
        {
            this.主路径格子 = 主路径格子;
            this.扩展格子 = 扩展格子;
            this.插入索引 = 插入索引;
    ...
    private List<可扩展格子> 寻找可扩展格子(int 连接数)
    {
        ...
        for (int i = 0; i < 最大索引; i++)
        {
            ...
            if (!同行 && !同列) continue;
            var 方向列表 = new System.Func<副本房间_基类, 副本房间_基类>[] { 周边房间_上, 周边房间_下, 周边房间_左, 周边房间_右 };
    ...
    private bool 检查同行(List<副本房间_基类> 格子列表) => 格子列表.Count <= 1 || 格子列表.All(格子 => 格子.行 == 格子列表[0].行);
    private bool 检查同列(List<副本房间_基类> 格子列表) => 格子列表.Count <= 1 || 格子列表.All(格子 => 格子.列 == 格子列表[0].列);
    ...
    private void 连接房间门(副本房间_基类 f1, 副本房间_基类 f2)
    {
    ...
    private void 生成一条支路(副本房间_基类 房间)
    {
    ...
    private struct 支路方向
    {
        public System.Func<副本房间_基类, 副本房间_基类> 方向方法;
        public string 方向名称;
        public 支路方向(System.Func<副本房间_基类, 副本房间_基类> 方法, string 名称) { 方向方法 = 方法; 方向名称 = 名称; }
    }
    ...
    private struct 主路支路配对
    {
        public 副本房间_基类 主路房间;
        public 副本房间_基类 支路房间;
        public 主路支路配对(副本房间_基类 主路房间, 副本房间_基类 支路房间)
        {
            this.主路房间 = 主路房间;
            this.支路房间 = 支路房间;
        }
    ...
    private List<支路方向> 获取可扩展方向(副本房间_基类 房间)
    {
    ...
    private List<副本房间_基类> 获取可连通房间(副本房间_基类 房间)
    {
        var 可连通房间 = new List<副本房间_基类>();
        foreach (var 周边房间 in 获取周边房间(房间))
            if (周边房间 != null && 周边房间.路径类型 == 副本房间路径类型.主路 && 周边房间 != 出口) 可连通房间.Add(周边房间);
        return 可连通房间;
    }

    private List<副本房间_基类> 获取未连通支路房间(副本房间_基类 房间)
    {
        var 未连通支路房间 = new List<副本房间_基类>();
    ...
    private List<副本房间_基类> 获取全部支路房间()
    {
        var 支路房间列表 = new List<副本房间_基类>();
        for (int 行 = 0; 行 < 副本房间行数; 行++)
        {
            for (int 列 = 0; 列 < 副本房间列数; 列++)
            {
    ...
    private List<副本房间_基类> 获取周边主路房间(副本房间_基类 支路房间)
    {
        var 周边主路房间 = new List<副本房间_基类>();
    ...
    private bool 是否已连通(副本房间_基类 支路房间, 副本房间_基类 主路房间)
    {
