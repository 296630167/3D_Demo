...
public class 副本场景 : 面板基类
{
    ...
    private int 计算技能伤害(角色类 攻击者, 角色类 被攻击者, 技能类 技能)
    {
        ...
        if (技能.伤害计算方式 == 技能伤害计算方式.战技)
        {
            ...
            伤害计算详情 += $"  - 计算公式为：普通攻击伤害 × (技能伤害倍率 + buff倍率) + 技能基础伤害\n";
            伤害计算详情 += $"  - 套入数值计算: {普通攻击伤害} × ({技能伤害倍率} + {buff倍率}) + {技能基础伤害}\n";
            伤害计算详情 += $"  - 最终战技伤害: {最终伤害}\n";
        }
        else if (技能.伤害计算方式 == 技能伤害计算方式.法术)
        {
            ...
            伤害计算详情 += $"  - 计算公式为：(技能基础伤害 + 魔法伤害修正 + buff倍率 + 其他) × (1 + 武器魔法加成)\n";
            伤害计算详情 += $"  - 套入数值计算: ({技能基础伤害} + {魔法伤害修正} + {buff倍率} + {其他}) × (1 + {武器魔法加成})\n";
            伤害计算详情 += $"  - 最终法术伤害: {最终伤害}\n";
        }
