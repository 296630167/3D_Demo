...
public class 辅助线绘制 : 基
{
    ...
    private Dictionary<string, GameObject> 填充区域对象字典 = new Dictionary<string, GameObject>();
    private Dictionary<string, GameObject> 填充圆形对象字典 = new Dictionary<string, GameObject>();
    #endregion
    ...
    public void 设置网格可见(bool 显示)
    {
        if (线条对象列表 == null) return;
        for (int i = 0; i < 线条对象列表.Count; i++)
        {
            var 线条 = 线条对象列表[i];
            if (线条 != null) 线条.SetActive(显示);
        }
    }
    ...
    public void 绘制移动范围区域(副本单位脚本 单位)
    {
        ...
        var 颜色 = new Color(0.6f, 0.6f, 0.6f, 0.8f);
    ...
    {
        const string 矩形区域名称 = "技能射程范围_合批";
        const string 圆形区域名称 = "技能攻击范围";
        ...
        var 颜色 = new Color(0.6f, 0.6f, 0.6f, 0.8f);
        绘制圆形区域(圆形区域名称, 坐标 + new Vector3(-0.005f, 0.01f, -0.005f), 半径, 颜色, 0.6f, (int)渲染层级.移动范围);
    }

    public void 绘制技能射程范围区域(副本单位脚本 单位, 技能类 技能)
    {
        const string 区域名称 = "技能射程范围_合批";
        if (单位 == null || 单位.单位 == null || 单位.单位.所在格子 == null)
        {
            取消绘制矩形区域(区域名称);
            取消绘制圆形区域("技能攻击范围");
            return;
        ...
        绘制技能攻击范围(单位.单位.所在格子.场景坐标, 技能);
    }

    public void 绘制群攻技能范围(Vector3 坐标,技能类 技能)
    {
        
    }
    public void 取消绘制技能攻击范围()
    {
        取消绘制矩形区域("技能射程范围_合批");
        取消绘制圆形区域("技能攻击范围");
    }
