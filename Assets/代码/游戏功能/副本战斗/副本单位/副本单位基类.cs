...
public class 副本单位脚本 : 基
{
    ...
    protected virtual IEnumerator 使用单体技能攻击敌人后携程(副本单位脚本 敌人, 技能类 技能)
    {
        ...
        yield return 启动携程(sj.副本场景.伤害计算携程(this, 敌人, 技能, (伤害) => 敌人.被攻击(伤害)));
        print("使用单体技能后结算结束");
    ...
    #endregion
    #region 群体技能攻击
    public virtual IEnumerator 使用范围技能攻击敌人携程(System.Collections.Generic.List<副本单位脚本> 敌人列表, 技能类 技能)
    {
        print(敌人列表?.Count);
        ...
        for (int i = 0; i < 敌人列表.Count; i++)
        {
            var 敌人 = 敌人列表[i];
            if (敌人 == null) continue;
            if (!敌人.单位.活着) continue;
            yield return 启动携程(敌人.被攻击(1));
        }
    }
    #endregion
    ...
    public virtual IEnumerator 被攻击(int 最终伤害)
    {
        ...
        if (单位.角色属性.当前血量 <= 0)
        {
            ...
            print("当前行动单位可移动范围更新完成");
            print("等待死亡动画播放");
            yield return new WaitForSeconds(1.1f);
            print("等待结束,准备启动单位阵亡携程");
            启动携程(sj.副本场景.单位阵亡(this));
            print("单位阵亡携程已启动");
        }
        print("被攻击流程结束");
    }
